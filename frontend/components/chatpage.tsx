...
export function Chatpage() {
  ...
  const renderMessage = (response: ResponseMessage, index: number) => (
    <div key={index} className="flex items-start gap-4">
      <Avatar className="w-8 h-8 border">
        <AvatarImage src={response.type === 'human' ? '/placeholder-user.jpg' : '/placeholder-bot.jpg'} />
        <AvatarFallback>{response.type === 'human' ? 'You' : 'SCU Chatbot'}</AvatarFallback>
      </Avatar>
      <div className="grid gap-1">
        <div className="font-bold">{response.type === 'human' ? 'You' : 'SCU Chatbot'}</div>
  ...
  const handleSubmit = (e: FormEvent) => {
    e.preventDefault();
    const userMessage: ResponseMessage = { type: "human", content: input };
    setResponses(prevResponses => [...prevResponses, userMessage]);
    ws.current?.send(JSON.stringify({ user_input: input, history: responses})); // Send message through WebSocket
